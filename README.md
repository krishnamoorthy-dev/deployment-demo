...
docker ps -  show live running containers
docker images - it show all the images
docker kill <container_id> - stop running container

docker build --platform=linux/amd64 -t my-deployment-demo .

docker run -d -p 8080:8080 my-deployment-demo
Note:
-d - detach mode. container will run without live log.
-p - port (ex. <exposing_port>:<container_port>)
...
Correct. A hypervisor creates and manages VMs.
